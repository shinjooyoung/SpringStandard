...
public class ValidationItemControllerV2 {
    ...
    * 컨트롤러에서만 사용가능
    * 검증기를 자동으로 적용
    * 호출될때마다 생성되서 전체 코드가 들어올떄 validator를 호출해서 검증해준다.
    * 단 사용하려면 메소드에 @Validated 를 파라미터 상에 넣어야 한다. 검증기를 실행하라는 에너테이션이라고 보면 된다.
    * */
    @InitBinder
    ...
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getPrice() != null && item.getQuantity() != null) {
            ...
            if(resultPrice < 10000) {
                bindingResult.addError(new ObjectError("item","가격 * 슈량의 합은 10,000 이상이어야 합니다. 현재 값 = " + resultPrice));
            }
        }
